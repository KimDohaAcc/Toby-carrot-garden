...
import { setSceneList } from "../store/slices/policeSlice.tsx";
import { setPoliceQuizClear } from "../store/slices/policeSlice.tsx";
...
const PoliceOffice = () => {
  ...
  const policeSceneList = useSelector<RootState, SceneList[]>( // 리덕스 스토어에서 장면 목록 가져오기
    (state: RootState) => state.police.sceneList // police 슬라이스의 sceneList 가져오기
  );

  const isQuizClear = useSelector<RootState, boolean>(
    (state: RootState) => state.police.quizClear
  ); // 리덕스 스토어에서 퀴즈 클리어 여부 가져오기

  const placeName = "police";
  ...
  const handleOnclickNextBtn = () => {
    ...
    setSceneIndex((prevIndex) => {
      ...
      setSceneType(policeSceneList[nextIndex].sceneType);
      return nextIndex;
    });
  ...
  const handleOnclickPrevBtn = () => {
    ...
    if (sceneIndex > 0) {
      setSceneIndex((currenIndex) => {
        ...
        setSceneType(policeSceneList[prevIndex].sceneType);
        return prevIndex;
      });
    } else {
      setSceneIndex(-1);
